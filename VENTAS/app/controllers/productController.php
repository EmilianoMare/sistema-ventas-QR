<?php
	...
	class productController extends mainModel{
		...
		public function listarProductoControlador($pagina,$registros,$url,$busqueda,$categoria,$marca="",$stock_filter="all",$precio_min="",$precio_max=""){
		...
		$marca=$this->limpiarCadena($marca);
		$stock_filter=$this->limpiarCadena($stock_filter);
		$precio_min=$this->limpiarCadena($precio_min);
		$precio_max=$this->limpiarCadena($precio_max);

		$tabla="";

		// Conversión deshabilitada: mostrar precios en ARS
		...
		if($marca!=""){
			$extraWhere .= " AND producto_marca LIKE '%$marca%'";
		}

		if($stock_filter=="in"){
			$extraWhere .= " AND producto_stock_total > 0";
		}elseif($stock_filter=="low"){
			$extraWhere .= " AND producto_stock_total > 0 AND producto_stock_total <= 10";
		}elseif($stock_filter=="out"){
			$extraWhere .= " AND producto_stock_total = 0";
		}

		if($precio_min!=""){
			$min = floatval(str_replace(',','.', $precio_min));
			$extraWhere .= " AND producto_precio_venta >= $min";
		}
		if($precio_max!=""){
			$max = floatval(str_replace(',','.', $precio_max));
			$extraWhere .= " AND producto_precio_venta <= $max";
		}

		if(isset($busqueda) && $busqueda!=""){

			$consulta_datos="SELECT $campos FROM producto INNER JOIN categoria ON producto.categoria_id=categoria.categoria_id WHERE (producto_codigo LIKE '%$busqueda%' OR producto_nombre LIKE '%$busqueda%' OR producto_marca LIKE '%$busqueda%' OR producto_modelo LIKE '%$busqueda%') $extraWhere ORDER BY producto_nombre ASC LIMIT $inicio,$registros";

			$consulta_total="SELECT COUNT(producto_id) FROM producto WHERE (producto_codigo LIKE '%$busqueda%' OR producto_nombre LIKE '%$busqueda%' OR producto_marca LIKE '%$busqueda%' OR producto_modelo LIKE '%$busqueda%') $extraWhere";

		}elseif($categoria>0){

	        $consulta_datos="SELECT $campos FROM producto INNER JOIN categoria ON producto.categoria_id=categoria.categoria_id WHERE producto.categoria_id='$categoria' $extraWhere ORDER BY producto.producto_nombre ASC LIMIT $inicio,$registros";

	        $consulta_total="SELECT COUNT(producto_id) FROM producto WHERE categoria_id='$categoria' $extraWhere";

	    }else{

			$consulta_datos="SELECT $campos FROM producto INNER JOIN categoria ON producto.categoria_id=categoria.categoria_id WHERE 1=1 $extraWhere ORDER BY producto_nombre ASC LIMIT $inicio,$registros";

			$consulta_total="SELECT COUNT(producto_id) FROM producto WHERE 1=1 $extraWhere";
			}
		    ...
		    if($total>=1 && $pagina<=$numeroPaginas){
				...
				foreach($datos as $rows){
					...
					$color_stock = $rows['producto_stock_total'] > 10 ? 'is-success' : ($rows['producto_stock_total'] > 0 ? 'is-warning' : 'is-danger');
					
					$tabla.='
						<div class="box product-item" style="padding: 0.75rem; margin-bottom: 0.5rem; background-color: #fafafa; border-left: 4px solid #3273dc;">
							<!-- Número y Imagen -->
							<div class="pi-meta" style="display:flex; align-items:center; gap:1rem;">
								<span style="font-weight: bold; color: #3273dc; font-size: 1.2em;">'.$contador.'</span>
								<figure class="image" style="width:64px; height:64px; margin:0;">
									<img src="'.($rows['producto_foto'] && is_file("./app/views/productos/".$rows['producto_foto']) ? APP_URL.'app/views/productos/'.$rows['producto_foto'] : APP_URL.'app/views/productos/default.png').'" alt="'.htmlspecialchars($rows['producto_nombre']).'" style="object-fit:cover; border-radius:3px; width:64px; height:64px; max-width:100%;">
								</figure>
							</div>

							<!-- Nombre y Detalles -->
							<div class="pi-details" style="flex:1 1 auto; min-width:0;">
								<p style="font-weight:bold; margin:0 0 .25rem 0;">'.$rows['producto_nombre'].'</p>
								<p style="margin:.25rem 0; font-size:.85em; color:#666; margin-bottom:0;">
									<strong>Cod:</strong> '.$rows['producto_codigo'].' | <strong>Cat:</strong> '.$rows['categoria_nombre'].'
								</p>
							</div>

							<!-- Stock -->
							<div class="pi-stock" style="flex:0 0 auto; text-align:center; min-width:0;">
								<span class="tag '.$color_stock.' is-light"><strong>Stock: '.$rows['producto_stock_total'].'</strong></span>
							</div>

							<!-- Precios -->
							<div class="pi-prices" style="flex:0 0 auto; text-align:center; min-width:0;">
								<p style="margin:.25rem 0;">
									<strong style="color:#27ae60;">ARS: $' . number_format($rows['producto_precio_venta'], 2, MONEDA_SEPARADOR_DECIMAL, MONEDA_SEPARADOR_MILLAR) . '</strong>
								</p>
							</div>

							<!-- Acciones -->
							<div class="pi-actions" style="display:flex; gap:.5rem; align-items:center; min-width:0;">
								<a href="'.APP_URL.'productPhoto/'.$rows['producto_id'].'/" class="button is-info is-light is-small" title="Cambiar foto">
									<span class="icon"><i class="far fa-image"></i></span>
								</a>
								<a href="'.APP_URL.'productUpdate/'.$rows['producto_id'].'/" class="button is-success is-light is-small" title="Editar">
									<span class="icon"><i class="fas fa-edit"></i></span>
								</a>
								<form class="FormularioAjax" action="'.APP_URL.'app/ajax/productoAjax.php" method="POST" autocomplete="off" style="display:inline; margin:0;">
									<input type="hidden" name="modulo_producto" value="eliminar">
									<input type="hidden" name="producto_id" value="'.$rows['producto_id'].'">
									<button type="submit" class="button is-danger is-light is-small" title="Eliminar">
		...
		/*----------  Listar marcas de productos (para filtros)  ----------*/
		public function listarMarcasProducto(){
			$consulta = "SELECT DISTINCT producto_marca FROM producto WHERE producto_marca<>'' ORDER BY producto_marca ASC";
