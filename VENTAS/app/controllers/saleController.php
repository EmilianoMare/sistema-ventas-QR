...
class saleController extends mainModel {
    ...
    public function buscarCodigoVentaControlador(){
        ...
        $log_msg = date('Y-m-d H:i:s')." | buscar_codigo recibido: '$producto'\n";
        $consulta_log = "SELECT * FROM producto WHERE (producto_codigo LIKE '%$producto%' OR producto_nombre LIKE '%$producto%' OR producto_marca LIKE '%$producto%' OR producto_modelo LIKE '%$producto%') ORDER BY producto_nombre ASC";
        $log_msg .= date('Y-m-d H:i:s')." | consulta ejecutada: $consulta_log\n";
        file_put_contents(__DIR__.'/../../logs/buscar_codigo_debug.log', $log_msg, FILE_APPEND);
        ...
        $datos_productos=$this->ejecutarConsulta("SELECT * FROM producto WHERE (producto_codigo LIKE '%$producto%' OR producto_nombre LIKE '%$producto%' OR producto_marca LIKE '%$producto%' OR producto_modelo LIKE '%$producto%') ORDER BY producto_nombre ASC");
    ...
    /*---------- Controlador autocomplete producto (JSON) ----------*/
    public function autocompleteProductoControlador(){
        $term = isset($_POST['term']) ? $this->limpiarCadena($_POST['term']) : '';
        if($term=='') return json_encode([]);

        $consulta = "SELECT producto_codigo, producto_nombre, producto_precio_venta, producto_stock_total FROM producto WHERE (producto_nombre LIKE '%$term%' OR producto_marca LIKE '%$term%' OR producto_modelo LIKE '%$term%' OR producto_codigo LIKE '%$term%') ORDER BY producto_nombre ASC LIMIT 8";
        $stmt = $this->ejecutarConsulta($consulta);
        $rows = $stmt->fetchAll();
        $results = [];
        foreach($rows as $r){
            $results[] = [
                'label' => $r['producto_nombre'].' ('.$r['producto_codigo'].')',
                'value' => $r['producto_codigo'],
                'precio' => number_format($r['producto_precio_venta'], MONEDA_DECIMALES, MONEDA_SEPARADOR_DECIMAL, MONEDA_SEPARADOR_MILLAR),
                'stock' => $r['producto_stock_total']
            ];
        }
    ...
    public function agregarProductoCarritoControlador(){
        ...
        if($this->verificarDatos("[a-zA-Z0-9- ]{1,70}",$codigo)){
            $alerta=[
                "tipo"=>"simple",
                "titulo"=>"Ocurrió un error inesperado",
                "texto"=>"El código del producto no coincide con el formato solicitado",
                "icono"=>"error"
            ];
            return json_encode($alerta);
            exit();
        ...
        if(empty($_SESSION['datos_producto_venta'][$codigo])){
            ...
            $detalle_total_raw = $detalle_cantidad * $campos['producto_precio_venta'];
            $detalle_total = round((float)$detalle_total_raw, MONEDA_DECIMALES);
    ...
    public function actualizarProductoCarritoControlador(){
        ...
        if(!empty($_SESSION['datos_producto_venta'][$codigo])){
            ...
            // Si la petición pide respuesta inline (actualización en página), devolver totales para actualizar el DOM sin recargar
            if(isset($_POST['inline_update']) && $_POST['inline_update']=='1'){

                // recalcular total de la venta (raw)
                $venta_total_raw = 0.00;
                foreach($_SESSION['datos_producto_venta'] as $p){
                    $venta_total_raw += (float)$p['venta_detalle_total'];
                }

                // guardar total en sesión con formato decimal punto (usado por el formulario oculto)
                $_SESSION['venta_total'] = round((float)$venta_total_raw, MONEDA_DECIMALES);

                $detalle_total_raw = (float) $_SESSION['datos_producto_venta'][$codigo]['venta_detalle_total'];
                $detalle_total_fmt = MONEDA_SIMBOLO . number_format($detalle_total_raw,MONEDA_DECIMALES,MONEDA_SEPARADOR_DECIMAL,MONEDA_SEPARADOR_MILLAR) . " " . MONEDA_NOMBRE;
                $venta_total_fmt = MONEDA_SIMBOLO . number_format($venta_total_raw,MONEDA_DECIMALES,MONEDA_SEPARADOR_DECIMAL,MONEDA_SEPARADOR_MILLAR) . " " . MONEDA_NOMBRE;

                $alerta = [
                    "tipo" => "inline",
                    "producto_codigo" => $codigo,
                    "venta_detalle_total" => $detalle_total_raw,
                    "venta_detalle_total_formatted" => $detalle_total_fmt,
                    "venta_total" => number_format($venta_total_raw,MONEDA_DECIMALES,'.',''),
                    "venta_total_formatted" => $venta_total_fmt
                ];
    ...
    public function registrarVentaControlador(){
        ...
        /*== Recompute venta total from session subtotals to ensure consistency ==*/
        $venta_total_raw = 0.00;
        ...
        /*== Formateando variables (usar valores numéricos redondeados) ==*/
        $venta_pagado = round((float)$venta_pagado, MONEDA_DECIMALES);
        $venta_total = round((float)$_SESSION['venta_total'], MONEDA_DECIMALES);

        // leer descuento enviado desde el formulario
        $venta_desc_tipo = isset($_POST['venta_desc_tipo']) ? $this->limpiarCadena($_POST['venta_desc_tipo']) : 'NINGUNO';
        $venta_desc_valor = isset($_POST['venta_desc_valor']) ? round((float)$_POST['venta_desc_valor'], MONEDA_DECIMALES) : 0.00;

        $venta_fecha=date("Y-m-d");
        $venta_hora=date("h:i a");

        // aplicar descuento al total (sin modificar subtotales de detalle)
        $venta_total_final = $venta_total;
        if($venta_desc_tipo === 'FIJO'){
            $venta_total_final = max(0, $venta_total - $venta_desc_valor);
        }elseif($venta_desc_tipo === 'PORC'){
            $venta_total_final = max(0, $venta_total - ($venta_total * ($venta_desc_valor/100)));
        }
        $venta_total_final = round((float)$venta_total_final, MONEDA_DECIMALES);

        /*== Calculando el cambio ==*/
        if($forma_pago=="CONTADO" && $venta_pagado < $venta_total_final){
            $alerta=[
                "tipo"=>"simple",
                "titulo"=>"Ocurrió un error inesperado",
        ...
        $venta_cambio = round($venta_pagado - $venta_total_final, MONEDA_DECIMALES);

        /*== Calculando total en caja ==*/
        $movimiento_cantidad = round($venta_pagado - $venta_cambio, MONEDA_DECIMALES);

        $total_caja = round((float)$datos_caja['caja_efectivo'] + $movimiento_cantidad, MONEDA_DECIMALES);
        ...
        /*== Manejo de CUENTA CORRIENTE (si corresponde) ==*/
        if ($forma_pago === "CUENTA_CORRIENTE") {

            $cliente_id = $_SESSION['datos_cliente_venta']['cliente_id'];
            $usuario_id = $_SESSION['id'];
            $deuda = $venta_total_final - $venta_pagado;

            if ($deuda > 0) {

                // Verificar si ya existe cuenta corriente
                $check_cc = $this->ejecutarConsulta("SELECT * FROM cuenta_corriente WHERE cliente_id = '$cliente_id'");

                // Crear cuenta si no existe
                if ($check_cc->rowCount() == 0) {
                    $this->ejecutarConsulta("
                        INSERT INTO cuenta_corriente (cliente_id, saldo) 
                        VALUES ('$cliente_id', 0)
                    ");
                }
                ...
                file_put_contents(__DIR__.'/../../logs/cc_mov_debug.log', date('Y-m-d H:i:s')." | cliente_id=$cliente_id | deuda=$deuda | venta=$codigo_venta | usuario_id=$usuario_id\n", FILE_APPEND);


                // Registrar movimiento con log de error
                try {
                    $this->ejecutarConsulta("
                        INSERT INTO cuenta_corriente_movimiento
                        (cliente_id, tipo, monto, detalle, fecha, usuario_id)
                        VALUES
                        ('$cliente_id', 'DEBE', '$deuda', 'Venta $codigo_venta', DATE(NOW()), '$usuario_id')
                    ");
                    file_put_contents(__DIR__.'/../../logs/cc_mov_debug.log', date('Y-m-d H:i:s')." | INSERT OK\n", FILE_APPEND);
                } catch (\Throwable $e) {
                    file_put_contents(__DIR__.'/../../logs/cc_mov_debug.log', date('Y-m-d H:i:s')." | INSERT ERROR: ".$e->getMessage()."\n", FILE_APPEND);
                }

                // Actualizar saldo
                $this->ejecutarConsulta("
                    UPDATE cuenta_corriente 
                    SET saldo = saldo + $deuda 
    ...
    public function listarVentaControlador($pagina,$registros,$url,$busqueda){
        ...
        // Modo monopuerto: no se usa conversión, mostrar totales en ARS
        ...
        if(isset($busqueda) && $busqueda!=""){
            ...
            // Buscar por código (parcial) o por nombre/apellido del cliente (parcial)
            $consulta_datos="SELECT $campos_tablas FROM venta INNER JOIN cliente ON venta.cliente_id=cliente.cliente_id INNER JOIN usuario ON venta.usuario_id=usuario.usuario_id WHERE (venta.venta_codigo LIKE '%$busqueda%' OR cliente.cliente_nombre LIKE '%$busqueda%' OR cliente.cliente_apellido LIKE '%$busqueda%') ORDER BY venta.venta_id DESC LIMIT $inicio,$registros";

            $consulta_total="SELECT COUNT(venta.venta_id) FROM venta INNER JOIN cliente ON venta.cliente_id=cliente.cliente_id WHERE (venta.venta_codigo LIKE '%$busqueda%' OR cliente.cliente_nombre LIKE '%$busqueda%' OR cliente.cliente_apellido LIKE '%$busqueda%')";
            ...
            foreach($datos as $rows){
                $tabla.='
                    <tr class="has-text-centered" >
                        ...
                        <td>
                            <strong>ARS:</strong> '.MONEDA_SIMBOLO.number_format($rows['venta_total'],MONEDA_DECIMALES,MONEDA_SEPARADOR_DECIMAL,MONEDA_SEPARADOR_MILLAR).'
                        </td>
                        <td>
                            ...
                            <button type="button" class="button is-link is-outlined is-rounded is-small btn-sale-options" onclick="print_invoice(\''.APP_URL.'app/pdf/remito.php?code='.$rows['venta_codigo'].'\')" title="Imprimir remito Nro. '.$rows['venta_id'].'" >
