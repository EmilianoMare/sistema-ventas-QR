<?php
	...
	if($datos_venta->rowCount()==1){
		...
		foreach($venta_detalle as $detalle){
			$desc = $ins_venta->limitarCadena($detalle['venta_detalle_descripcion'],80,"...");
			$pdf->Cell(100,7,iconv("UTF-8", "ISO-8859-1",$desc),'L',0,'L');
			$pdf->Cell(15,7,iconv("UTF-8", "ISO-8859-1",$detalle['venta_detalle_cantidad']),'L',0,'C');
			$pdf->Cell(32,7,iconv("UTF-8", "ISO-8859-1",MONEDA_SIMBOLO.number_format($detalle['venta_detalle_precio_venta'],MONEDA_DECIMALES,MONEDA_SEPARADOR_DECIMAL,MONEDA_SEPARADOR_MILLAR)),'L',0,'R');
			$pdf->Cell(34,7,iconv("UTF-8", "ISO-8859-1",MONEDA_SIMBOLO.number_format($detalle['venta_detalle_total'],MONEDA_DECIMALES,MONEDA_SEPARADOR_DECIMAL,MONEDA_SEPARADOR_MILLAR)),'LR',0,'R');
			$pdf->Ln(7);
		}
		...
		$pdf->SetFillColor(23,83,201);
		$pdf->SetTextColor(255,255,255);
		$pdf->Cell(70,7,'TOTAL A PAGAR',1,1,'C',true);
		$pdf->SetX($tx);
		$pdf->SetFont('Arial','B',12);
		$pdf->Cell(70,10,MONEDA_SIMBOLO.number_format($datos_venta['venta_total'],MONEDA_DECIMALES,MONEDA_SEPARADOR_DECIMAL,MONEDA_SEPARADOR_MILLAR).' '.MONEDA_NOMBRE,1,1,'C',true);

		$pdf->SetTextColor(39,39,51);
		$pdf->SetFont('Arial','',9);
		$pdf->SetXY(17,$pdf->GetY()+4);
		$pdf->Cell(100,7,'TOTAL PAGADO',0,0,'L');
		$pdf->Cell(62,7,MONEDA_SIMBOLO.number_format($datos_venta['venta_pagado'],MONEDA_DECIMALES,MONEDA_SEPARADOR_DECIMAL,MONEDA_SEPARADOR_MILLAR).' '.MONEDA_NOMBRE,0,1,'R');
		$pdf->Cell(100,7,'CAMBIO',0,0,'L');
		$pdf->Cell(62,7,MONEDA_SIMBOLO.number_format($datos_venta['venta_cambio'],MONEDA_DECIMALES,MONEDA_SEPARADOR_DECIMAL,MONEDA_SEPARADOR_MILLAR).' '.MONEDA_NOMBRE,0,1,'R');
		...
		$pdf->SetDrawColor(23,83,201);
		$barcodeY = $pdf->GetY()+8;
		$pdf->Code128(72,$barcodeY,$datos_venta['venta_codigo'],70,20);
		$pdf->SetXY(12,$barcodeY+23);
